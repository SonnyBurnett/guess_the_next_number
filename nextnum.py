...
def get_user_numbers():
    return_list = []
    val = "xxx"
    while val != "" and len(val) < 6:
        val = input("Getal: ")
        if val != "":
            if len(val) >= 6:
                return_list = [int(x) for x in val.split()]
                break
            else:
...
def health_check():
    print("Health check. Expecting: -9, 24, 48, 634")
    find_solution([-14, -25, -31, -31, -24])
    find_solution([22, 27, 29, 28])
    find_solution([3, 12, 6, 24, 12])
    find_solution([19, 34, 94, 274])
    print()
...
def main():
    ...
    while True:
        ...
        if len(user_list) < 1:
            print('bye!  <Ctrl><F5> to start again, or press green triangle icon on the left')
            break
        else:
